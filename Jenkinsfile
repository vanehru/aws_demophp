pipeline{
    agent any
    parameters {
        string(name: 'git_url', description: 'url of github', defaultValue: 'https://github.com/vanehru/aws_demophp.git')
    ...
    stages{
            ...
            stage('docker image stages'){
                ...
                stages{
                    ...
                    stage('docker build'){
                        steps{
                        sh """docker build -t public.ecr.aws/q5y5m4j7/demo/php/${env_name}:${BUILD_NUMBER} -f Dockerfile . """
                        }
                    }
